...
// NewSilentEnv sets up a silent environment
func NewSilentEnv(cfg ...interface{}) *Env {
	var e env
	e.silent = true // toggle
	return e.Configure(cfg...)
}

// Configure sets up the basic environment
func (e *env) Configure(cfg ...interface{}) *Env {
...
func (e *env) helpTag(cfg interface{}) {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		var env struct{ Order, Require, Environ, Alias string }
		...
		fmt.Printf("%-5s [%-1s%-1s%-1s] ", env.Alias, env.Order, env.Require, env.Environ)
